...
int SendUDP::Execute() {
	...
	if (mAddress > 0) {
		...
		//受信データ量
		int length = GetNetWorkDataLength(mHandle);

		result = NetWorkSendUDP(mHandle, mIp, mSendPort, mBuffer, mAddress);
